...
def test_crypto_selftest_rejects_each_broken_primitive(
...
def test_http_probe_and_log_tail_boundaries(
    ...
    monkeypatch.setattr(
        startup_selftest.urllib.request,
        "urlopen",
        lambda *_args, **_kwargs: _Response(200, b'{"status":"ok"}'),
    )
    ...
    monkeypatch.setattr(
        startup_selftest.urllib.request,
        "urlopen",
        lambda *_args, **_kwargs: _Response(503, b"failed"),
    )
...
def test_frozen_probe_success_exit_and_timeout(
    ...
    monkeypatch.setattr(
        startup_selftest.subprocess, "Popen", lambda *_args, **_kwargs: success
    )
    ...
    monkeypatch.setattr(
        startup_selftest.subprocess, "Popen", lambda *_args, **_kwargs: no_frontend
    )
    assert (
    ...
    exited = _Process([7])
    monkeypatch.setattr(
        startup_selftest.subprocess, "Popen", lambda *_args, **_kwargs: exited
    )
    ...
    monkeypatch.setattr(
        startup_selftest.subprocess, "Popen", lambda *_args, **_kwargs: timed_out
    )
    ticks = iter([0.0, 0.0, 11.0])
    # 不能修改标准库 time 模块对象本身；完整测试套件中公文排版清理线程也在
    # 使用同一模块，跨线程猴子补丁会消耗计时序列并造成不确定失败。
    monkeypatch.setattr(
        startup_selftest,
        "time",
    ...
    monkeypatch.setattr(
        startup_selftest,
        "_read_json",
        lambda _url: (_ for _ in ()).throw(OSError("未监听")),
    )
