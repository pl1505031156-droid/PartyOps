...
def test_windows_installer_is_chinese_branded_and_preserves_custom_paths() -> None:
    ...
    assert "function SetAdministratorOwnerWithFallback" in installer
    assert "INSTALL_DIR_TREE_OWNER_DENIED" in installer
    ...
    assert "VersionInfoVersion=1.4.5.3" in installer
...
def test_arm64_native_package_is_host_wrapped_then_chroot_tested() -> None:
    ...
    assert "PARTYOPS_NATIVE_BUILD_BASE=/tmp/partyops-build-arm64-package" in script
    assert "PARTYOPS_NATIVE_BUILD_BASE=/tmp/partyops-native-build-arm64" in script
