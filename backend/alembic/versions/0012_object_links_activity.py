...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_object_links_source",
        "object_links",
        ["source_type", "source_id"],
    )
    op.create_index(
        "ix_object_links_target",
        "object_links",
        ["target_type", "target_id"],
    )
    op.create_index("ix_object_links_link_type", "object_links", ["link_type"])
    op.create_index("ix_object_links_created_by", "object_links", ["created_by"])
    ...
    op.create_index(
        "ix_activity_object_time",
        "activity_events",
        ["object_type", "object_id", "happened_at"],
    )
    op.create_index("ix_activity_events_event_code", "activity_events", ["event_code"])
    op.create_index("ix_activity_events_actor_id", "activity_events", ["actor_id"])
    op.create_index(
        "ix_activity_events_correlation_id",
        "activity_events",
        ["correlation_id"],
    )
...
def downgrade() -> None:
    ...
    op.execute("DELETE FROM schema_release_notes WHERE revision = '0012'")
