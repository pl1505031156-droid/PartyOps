...
PartyOps `1.4.3-rc.9` 按“包格式 + 处理器架构”交付，不按 CPU 型号或银河麒麟小版本重复制作安装包。银河麒麟桌面 V10 SP1 的 2107、2203、2303、2403、2503 使用同一套选包和安装后自检规则。
...
| 输出 | 常见处理器 | 下载文件 |
| --- | --- | --- |
| `aarch64` | 飞腾 D2000/FT-2000、麒麟 9000C/9006C/990、鲲鹏 | `PartyOps_1.4.3-rc.9_linux_arm64.deb` |
| `x86_64` | 海光 C86、兆芯、Intel、AMD | `PartyOps_1.4.3-rc.9_linux_amd64.deb` |
| `loongarch64`、`sw64`、`riscv64` | 龙芯、申威、RISC-V | rc.9 暂无对应包，不得强制安装 AMD64/ARM64 包 |

CPU 名称只用于帮助新手理解，最终以 `uname -m` 为准。截图所示的飞腾 D2000 与 HUAWEI Kirin 9000C 设备应在 `uname -m` 返回 `aarch64` 时选择同一个 ARM64 DEB。

## 系列版本边界

| 银河麒麟桌面版本 | rc.9 处理方式 | 结论边界 |
| --- | --- | --- |
| V10 SP1 2107 / 2203 | 使用同架构 DEB，并由 APT 依赖门禁与安装后自检决定是否提交安装 | 未取得对应真机，不能写成真机通过 |
| V10 SP1 2303 / 2403 | 官方组件矩阵为 glibc 2.31、systemd 245；rc.9 DEB 仅要求 glibc 2.17 | 静态兼容与 QEMU 动态门禁通过，未取得商业系统真机 |
...
```bash
sudo install -m 0644 ./PartyOps_1.4.3-rc.9_linux_arm64.deb /var/tmp/partyops.deb
...
rc.9 已把 `.desktop` 的 `Exec` 和 `TryExec` 固定到 `/bin/bash`，由脚本再进入 `/opt/partyops` 固定运行时；启动器在写任何配置前使用 `getent passwd` 恢复当前账号的 `HOME`，然后立即创建 `~/.config/partyops/desktop-launch.log` 和启动诊断。安装器把 `HOME`、`XDG_CONFIG_HOME`、`XDG_RUNTIME_DIR`、`DBUS_SESSION_BUS_ADDRESS` 传给真实桌面用户后再设置可信元数据，并保留桌面文件可执行位兜底。
