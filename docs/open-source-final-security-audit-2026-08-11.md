...
6. 仓库分支/标签已完成历史重写，但 GitHub 的旧 PR 缓存引用仍需由 GitHub Support 清除后复核。
...
| 原报告主张 | 判定 | 当前证据与处理 |
| --- | --- | --- |
| 三份现场修复文档泄露真实机器标识与部署路径 | **属实** | 当前树及本地全部分支/标签已经重写，五个目标路径的历史命中均为 0；远端分支/标签强制更新后，还需按 GitHub 官方流程清除旧 PR 缓存引用。 |
...
| 编号 | 位置 | 风险 | 修复与状态 |
| --- | --- | --- | --- |
| SEC-001 | Git 历史中的三份现场修复文档、内部规划 DOCX、机器路径脚本 | 删除当前文件并不能删除 GitHub 已公开 Blob；真实机器标识、目录拓扑和开发机路径仍可被历史检索。 | 已用 `git-filter-repo 2.47.0 --sensitive-data-removal` 重写全部本地分支/标签，五个路径历史命中为 0；远端更新后仍需联系 GitHub Support 清理旧 PR 缓存引用。**代码历史已关闭，平台缓存待关闭**。 |
...
| DEV-001 | 旧机器专用构建脚本/部署文档 | 开发者用户名和绝对路径造成隐私暴露且使外部贡献者无法复现。 | 删除机器专用脚本，文档改为变量和相对路径；**当前树与 Git 分支/标签历史已修复**。 |
...
1. GitHub 远端所有相关分支和标签完成隐私历史重写，并由 GitHub Support 清理旧 PR 缓存引用后复核旧 Blob 不可达。
...
GitHub 官方说明：强制推送重写历史后，旧提交仍可能经 PR 引用和缓存视图访问，完整清除需要联系 GitHub Support；参见 [Removing sensitive data from a repository](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/removing-sensitive-data-from-a-repository)。
