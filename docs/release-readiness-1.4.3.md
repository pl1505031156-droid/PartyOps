...
5. Win7 x64/x86 对最终 PE、UCRT、API-set、Python 3.8、OpenSSL、RSA 与 Fernet 做成品运行门禁；不再把 KB 登记名当作唯一能力证明。
...
Windows 7、国产 Linux 与 macOS 缺少用户真机交互验收，不宣称已获真实使用验证。Windows 安装器没有 Authenticode 商业证书，可能触发 SmartScreen。macOS PKG 只有 ad-hoc 签名，没有 Developer ID Application/Installer 与 Apple 公证；核对哈希后按官网“仍要打开”步骤操作，不得关闭 Gatekeeper。永久消除提示需要正式证书、公证与 staple。
