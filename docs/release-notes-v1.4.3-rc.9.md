...
macOS 未在本轮 Windows/Linux 修复环境中生成新的原生 rc.9 包；官网继续把已验证的 rc.8 ARM64/x86_64 PKG 标为“macOS 当前版”，不会改名或伪造成 rc.9。

## 验收边界

- 后端、前端、版本一致性、Windows 安装脚本真实编译和 Linux shell 语法门禁已通过；最终安装包的大小、SHA-256、平台格式及线上全量回读以发布就绪记录为准。
...
- macOS 当前包没有 Developer ID 与公证，不关闭 Gatekeeper；本轮不发布 macOS 应用内更新包。
