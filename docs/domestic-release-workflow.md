...
安装包只允许上传到项目方 Cloud Studio 已挂载的受控下载后台，公开路径必须位于 `/downloads/`。不得调用 WorkBuddy，也不得把聊天附件、临时网盘、GitHub 代理或未经完整回读的镜像写入官网主下载链。
...
1. 登录项目方现有 Cloud Studio 工作区；需要验证码时只在官方登录页完成，不把验证码或会话凭据写入仓库。
2. 打开已挂载的安装包管理/上传后台，确认目标路径为受控 `/downloads/`。
3. 每次只选择冻结清单中的文件；不得上传调试目录、旧版安装包、临时归档或私钥。
4. 上传结束后记录后台返回的对象标识、公开 HTTPS 地址和北京时间。
5. 公开 URL 的路径必须精确为 `/downloads/<冻结文件名>`；若后台返回不同文件名、登录页、临时签名 URL 或 HTML 中转页，该文件不合格。
6. Cloud Studio 目录只保留当前公开版本的主安装包和必要校验文件；历史版本由 GitHub Release 保留审计记录。
...
`v1.4.3-rc.9` 曾使用第三方后台静态应用作为国内线路。该方式仅保留在历史发布记录中用于审计，从 `v1.4.4` 起不再调用、不再写入源码或官网主下载链，也不得作为 Cloud Studio 上传的替代方案。
