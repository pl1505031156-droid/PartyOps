...
最后核对：2026-08-18（北京时间，UTC+8）。本文只描述当前 `1.4.3-rc.7` / 数据库 `0019`；旧版本文档仅用于迁移追溯。

> `v1.4.3-rc.7` 是未签名候选版，不是稳定生产版。Windows 7 与国产 Linux 制品标为“未真机验证”；Windows 7 已停止系统级安全维护，只能在受控局域网使用。
...
dpkg-deb --info PartyOps_1.4.3-rc.7_linux_amd64.deb
```

哈希不一致时停止安装。当前 `rc.7` Windows 文件未做 Authenticode 正式签名，SmartScreen 显示未知发布者属于已知限制，不应误称为正式签名版本。

### 2.2 Windows 10/11 x64

1. Windows 10/11 运行 `PartyOps_1.4.3-rc.7_windows_amd64.exe`，程序目录与业务数据目录都可选择本机固定磁盘路径，例如 `E:\PartyOps` 与 `D:\PartyOps-数据`。Win7 必须改用文件名带 `windows7_amd64` 或 `windows7_x86` 的专用包。
...
```bash
uname -m
sudo apt install ./PartyOps_1.4.3-rc.7_linux_amd64.deb
# aarch64 / ARM64 使用 PartyOps_1.4.3-rc.7_linux_arm64.deb
```
...
# aarch64 使用 PartyOps-1.4.3-0.rc.7.1.aarch64.rpm
```
