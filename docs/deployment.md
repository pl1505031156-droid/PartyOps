...
最后核对：2026-08-21（北京时间，UTC+8）。本文只描述当前 `1.4.4` / 数据库 `0020`；旧版本文档仅用于迁移追溯。

> `v1.4.4` 按平台标记 `preview` 或 `unavailable`，不是全平台稳定生产版。Windows 7、国产 Linux 与 macOS 制品标为“未完成用户真机交互验收”；Windows 7 已停止系统级安全维护，只能在受控局域网使用。
...
dpkg-deb --info PartyOps_1.4.4_linux_amd64.deb
```

哈希不一致时停止安装。当前 `1.4.4` Windows 文件未做 Authenticode 正式签名，SmartScreen 显示未知发布者属于已知限制，不应误称为正式签名版本。

### 2.2 Windows 10/11 x64

1. Windows 10/11 运行 `PartyOps_1.4.4_windows_amd64.exe`，程序目录与业务数据目录都可选择本机固定磁盘路径，例如 `E:\PartyOps` 与 `D:\PartyOps-数据`。Win7 必须改用文件名带 `windows7_amd64` 或 `windows7_x86` 的专用包。
...
```bash
uname -m
sudo apt install ./PartyOps_1.4.4_linux_amd64.deb
# aarch64 / ARM64 使用 PartyOps_1.4.4_linux_arm64.deb
```
...
# aarch64 使用 PartyOps-1.4.4-1.aarch64.rpm
```
...
Apple 芯片选择 `PartyOps_1.4.4_macos_arm64.pkg`，Intel 芯片选择 `PartyOps_1.4.4_macos_x86_64.pkg`。当前 PKG 只有 ad-hoc 签名，没有 Developer ID Installer/Application 与 Apple 公证；先核对 SHA-256，再在 Finder 中 `Control` 点按 PKG →“打开”，或在“系统设置 → 隐私与安全性”选择对应的“仍要打开”。不得关闭 Gatekeeper。
