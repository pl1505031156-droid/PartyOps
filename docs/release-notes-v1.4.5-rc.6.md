...
最后更新：2026-08-30（北京时间，UTC+08:00）
...
- 国产 Linux 启动修复：根据银河麒麟 V10 SP1 2403、麒麟 9006C 用户提供的 rc.4
  `CONFIG_INVALID` 诊断，移除 `desktop-launcher.sh` 与 `start.sh` 对原始用户配置的
  直接 `source`。配置现在由内置向导按白名单解析、角色/端口/路径校验后生成 0600
  一次性环境文件；截断、引号不闭合、重复键、错误角色和非绝对数据目录均写入精确
  诊断并自动打开预选原角色的修复向导，原业务数据不删除。该链路由同一代码覆盖
  ARM64 与 AMD64 包。
