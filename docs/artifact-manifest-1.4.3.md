...
| `PartyOps-UOS-1.4.3-rc.2-build-kit.zip` | amd64/arm64 严格离线构建套件；不是 DEB | 338,686,331 字节；SHA-256 `93d6bfeb79952168c3a7f888f4d8fb98ac1b562a754ca8e3e4984ba0fddbff87` |
...
UOS ZIP 已独立复核：amd64/arm64 各 66 个 wheel，每个架构只有一个 `cryptography-50.0.0`；arm64 使用 `manylinux_2_28_aarch64` 轮子；禁止分发的私钥条目为 0。当前数值对应 rc.2 严格闭包候选，若最终源码提交变化必须重建候选 JSON 并再次确认 ZIP 哈希。
