...
| 验证项 | 结果 | 证据/说明 |
| --- | --- | --- |
| 后端全量 | 201 passed | 覆盖协同文件、权限撤销、分块、显式 finalize、扫描并发与既有业务 |
| 后端覆盖率 | 73.86% | 2026-08-09 在精确发布副本执行 `pytest --cov=app`；低于 90% 门槛 |
| 前端单元 | 41 passed | 新增文档格式、64 MiB、流式读取、中文错误、安全 Markdown 和分页标记回归 |
| 前端覆盖率 | 行/语句 5.48% | 2026-08-09 再次执行 `vitest --coverage`；低于 90% 门槛 |
...
| 依赖审计 | 通过/有例外 | pnpm 生产依赖无已知漏洞；Python 除既有 cryptography 精确例外外无已知漏洞 |
