...
RELEASE_NOTES = [
    ...
    "修复麒麟 ARM64 共享库错误执行权限与服务无限重试，官网按系统和处理器品牌精确引导选包",
    "修复全平台桌面入口静默失败，页面或默认浏览器无法打开时显示中文诊断与可复制地址",
    "补齐 Windows 冻结向导运行依赖，并以最终 EXE 启动自检阻断安装后无法打开的制品",
    "新增 macOS 11 及以上 Apple 芯片与 Intel 原生安装、登录恢复和系统内升级",
]
