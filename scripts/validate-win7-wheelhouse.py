...
def read_wheels(wheelhouse: Path, architecture: str) -> dict[str, tuple[Version, Path]]:
    wheels: dict[str, tuple[Version, Path]] = {}
    ...
    for path in sorted(wheelhouse.glob("*.whl")):
        ...
        wheels[name] = (Version(raw_version), path)
    if not wheels:
...
def validate_evidence(
    config_path: Path, evidence_root: Path, wheels: dict[str, tuple[Version, Path]]
) -> None:
    config = json.loads(config_path.read_text(encoding="utf-8"))
    for raw_name, component in config.get("components", {}).items():
        name = canonicalize_name(raw_name)
        wheel = wheels.get(name)
        required_version = Version(component["required_build_version"])
        if wheel is None:
        ...
        if evidence.get("version") != str(required_version):
...
def validate_capabilities(
    wheels: dict[str, tuple[Version, Path]], architecture: str
) -> None:
    names = set(wheels)
