...
| `1.4.3-rc.6` | 修复 Windows PowerShell 5.1 安装失败、Win7 误装、麒麟 ARM64 共享库权限及全平台桌面入口静默失败；官网按系统和处理器品牌选包 | 当前候选；未签名、Win7/国产 Linux 未真机验证 |
...
- 一次性桥接：由于 rc.2 的发布签名私钥已经不可恢复，rc.2 及更早版本需最后一次从官网运行与本机系统、CPU 架构匹配的当前 rc.6 安装器并选择原位升级；无需卸载且默认保留数据。rc.3 及后续版本走系统内快速升级。
