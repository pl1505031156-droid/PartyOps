...
[![Source](https://img.shields.io/badge/source-v1.4.5--rc.2-c58b3d?style=for-the-badge)](docs/release-notes-v1.4.5-rc.2.md)
...
> [!IMPORTANT]
> 当前开发源码为 `1.4.5-rc.2`，数据库模式为 `0023`；新增本机一次性公文规范排版，并集中修复运行身份事务、协同公布地址、文件预览/打开、发展党员预测、麒麟安装进度、跨平台启动和卸载数据保护。rc.2 尚未通过正式 Ed25519 密钥、各平台原生制品和公开回读全部门禁，因此不得把源码候选误称为已发布安装包；当前公开下载仍为 `v1.4.5-rc.1`。详见[rc.2 发布说明](docs/release-notes-v1.4.5-rc.2.md)和[rc.2 机器可读支持矩阵](docs/support-matrix-1.4.5-rc.2.json)。
...
| `1.4.5-rc.1` | 新增三会一课、中心组学习、发展党员三轨时间轴、多文件可恢复资料、本机硬件检测、12 档模型推荐和身份重新配置 | 当前公开版本；支持等级、签名和真机验证状态以发布清单为准 |
| `1.4.5-rc.2` | 新增本机公文规范排版，修复协同地址、文件打开、发展党员预测、麒麟安装、跨平台启动与卸载保护 | 开发候选，尚未公开；所有目标在原生制品和正式签名门禁前均为 unavailable |
...
当前公开版步骤见[1.4.5-rc.1 安装、升级与回滚](docs/upgrade-1.4.5-rc.1.md)；rc.2 迁移边界见[1.4.5-rc.2 升级与回滚](docs/upgrade-1.4.5-rc.2.md)。通用说明另见[备份恢复手册](docs/backup-restore.md)和[长期运行手册](docs/operations-runbook.md)。
