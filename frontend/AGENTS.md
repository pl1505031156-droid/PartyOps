...
## PartyOps 公文排版工具持久决策（2026-08-29）

- 用户选定 Product Design 第 1 案“三栏内嵌工作台”作为公文排版页面的视觉真值；继续复用现有米白、朱红、炭黑、宋体标题与 2px 圆角主题，不创建第二套系统主题。
- `PartyOps.DocumentFormatter.Source` 的六大功能和 25 项产品能力是功能兼容底线；界面重排不得删除、改名、合并或弱化现有功能、参数、输入输出与错误语义。
- 排版工具只作为 PartyOps 内置页面使用；处理过程不得打开 WinForms、Word、WPS 或其他系统外业务窗口。系统文件/目录选择器属于允许的原生选择入口。
- 九个平台统一使用安装包随附的本地引擎，不要求用户安装 Office、WPS、LibreOffice 或联网文档服务；兼容模式只表达输出目标。
